...
def login():
    global logged_in, login, password
    if request.method.upper() == 'POST':
        login = request.form['login']
        password = request.form['password']
    if login == 'stavrosvarvatsoulis' and password == '355':
        logged_in = True
        headers, data = read_file('states.csv')
        states = [row[0] for row in data]
        # definitely do not uncomment below. return ends execution of the current function, so rendering
        # choose_state.html will never even happen return f'User {login} has successfully logged in.
